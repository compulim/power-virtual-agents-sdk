...
import { UUID_REGEX, fallback, object, parse, regex, string, union, unknown, value, type StringSchema } from 'valibot';
...
app.post('/environments/:environmentId/bots/:botId/test/conversations/:turn', async (req, res) => {
  ...
  const { botId, environmentId, turn: initialTurn } = parse(RequestParamsSchema, req.params);
  let turn = initialTurn;

  for (let index = 0; index < 100; index++) {
    const url = new URL(
      `/environments/${encodeURI(environmentId)}/bots/${encodeURI(botId)}/test/conversations/${turn}`,
      BASE_URL
    );

    url.search = new URL(req.url).search;

    const proxyRes = await fetch(url, { headers: { authorization } });
    ...
    res.setHeader('content-type', 'text/event-source');
